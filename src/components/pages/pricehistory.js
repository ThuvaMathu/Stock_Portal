...
function Pricehistory(props) {
  ...
  const API_KEY = "VZR54LH35Z5MKXIT";
  const API_KEY2 = "demo";
  ...
  const handlesearch = () => {
    ...
    if (searchdate != null) {
      ...
      let endDate = new Date(
        Math.max(...showdata.map((e) => new Date(e.date)))
      );
      let datefilter = showdata.filter(function (x) {
        return new Date(x.date) >= startDate && new Date(x.date) <= endDate;
      });
    ...
    <>
      <div>
        ...
        <Paper className="show-stock">
          <p>
            Showing Stocks for the <span>{name}</span>{" "}
          </p>
        </Paper>
    ...
    return (
      <>
        <div>
          <Toolbar>
            <h1> Price history {"\u00a0\u00a0"}</h1>{" "}
            <HistoryIcon className="stock-head" />
          </Toolbar>
            ...
            <Grid container justifyContent="center" spacing={1}>
              <Grid item>
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField {...params} label="Search symbol" />
                  )}
                />
              </Grid>
              <Grid item>
          ...
          <div className="margintop10px">
            {open && <Pricehistory id={value} name={searchname} />}
          </div>
        </div>
      </>
    );
